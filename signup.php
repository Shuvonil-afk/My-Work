...
if(isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($sql) > 0) {
        ...
        $customer_id = rand(100000, 999999);  // This generates a 6-digit random number

        // Insert new user with unique customer ID
        $insert = mysqli_query($con, "INSERT INTO users (fname, lname, email, password, contactno, customer_id) VALUES ('$fname', '$lname', '$email', '$password', '$contact', '$customer_id')");
        if ($insert) {
            echo "<script>alert('Registered successfully. Your Customer ID is: $customer_id');</script>";
